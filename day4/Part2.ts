...
class Part2 {
    ...
    private allRollsRemoved: boolean;
    ...
    private rollCanBeAccessed(x: number, y: number): boolean {
        ...
        // console.log();
        if (nearbyRolls < 4) {
            this.input[y]?.splice(x, 1,".");
            this.display[y]?.splice(x, 1,".");
        }
    ...
    private solve() {
        ...
        for (let y = 0; y < this.maxY; y++) {
            for (let x = 0; x < this.maxX; x++) {
                if (this.input[y]![x] === "@" && this.rollCanBeAccessed(x, y)) {
                    this.totalRolls++;
                    cleanRun = false;
                };
            }
        ...
        if (cleanRun) this.allRollsRemoved = true;
    }
